...
const App: React.FC = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse<BitcoinWellRow>(file, {
      ...
      complete: (results) => {
        ...
        // Generate a CSV string from the Koinly rows
  ...
  // Converts a single Bitcoin Well row into a Koinly-compatible row
  const convertRow = (row: BitcoinWellRow): KoinlyRow => {
    ...
    // Map the transaction type and amounts based on the order type
  ...
  // Generates a CSV string from an array of Koinly rows
  const generateKoinlyCsv = (rows: KoinlyRow[]): string => {
    ...
    rows.forEach((row) => {
      // Escape values that include commas or quotes
